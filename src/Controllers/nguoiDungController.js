...
const deleteUser = async (req, res) =>{
    try{
        ...
        const checkUser = await prisma.NguoiDung.findFirst({where: {id_nguoidung: +id}});
        const isBinhLuanExist = await prisma.BinhLuan.findFirst({where: {id_nguoidung: +id}});
        const isDatPhongExist = await prisma.DatPhong.findFirst({where: {id_nguoidung: +id}});
        if(!isBinhLuanExist && !isDatPhongExist){
            if(checkUser){
                ...
                failCode(res, null, "Không tồn tại user có id: " +id);
            }
        }else{
            failCode(res, isBinhLuanExist, "Xóa id_nguoidung: " + id + " ở bình luận và phòng để thực hiện tính năng này");
        }
    }catch(err){
        errorCode(res, err, "Lỗi backend");
    }
